...
int CheckGLError(const char* file, int line)
{
	...
	{
		const char* sError = GetGLErrorString(glErr);//const GLubute* sError = gluErrorString(glErr);

		if (sError)
			cout << "GL Error #" << glErr << "(" << sError/*gluErrorString(glErr)*/ << ") " << " in File " << file << " at line: " << line << endl;
		else
